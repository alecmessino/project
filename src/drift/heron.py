...
def _tone_fn(field: _Field):
    ...
    def tone(x: float, y: float) -> float:
        v = 0.94
        v -= 0.16 * _band(_dist_to(_BACK_LINE, x, y), 22.0)      # light along the back
        v -= 0.13 * _band(_dist_to(_COVERT_LINE, x, y), 11.0)    # the covert streak
        v += 0.16 * _band(_dist_to(_NECK_FRONT, x, y), 16.0)     # dark margin down the throat
        v += 0.13 * _band(_dist_to(_BREAST_LINE, x, y), 30.0)    # the breast carries the weight
        ...
        v *= 0.76 + 0.24 * _ramp(0.4, 6.0, field.edge(x, y))
...
def _ink(lines, seq: _Seq, tone, gain: float = 1.0, hold: float = 0.20, gape: float = 0.5,
...
def build():
    ...
    def leg_tone_fn(field):
        def f(x, y):
            v = 0.95 + 0.04 * math.sin(y * 0.033) - 0.05 * _ramp(1240.0, 1340.0, y)
            return max(0.0, min(0.99, v * (0.52 + 0.48 * _ramp(0.3, 3.6, field.edge(x, y)))))
        return f

    # ── the primary passes: long, form-following, one family that owns the whole bird ──
    runs = _ink(_streams(body, _cross(_flow, math.radians(23)), 3.6), seq, tone, hold=0.15)
    # ── cross-hatch, in the shadow only, off the flow by ~70° and ~35° ──
    runs += _ink(_streams(body, _cross(_flow, math.radians(-42)), 5.2), seq,
                 _shadow(tone, 0.34), hold=0.24, gape=0.42, wander=0.3)
    runs += _ink(_streams(body, _cross(_flow, math.radians(78)), 7.2), seq,
                 _shadow(tone, 0.50), hold=0.30, gape=0.4, wander=0.3)
    # ── the wing's own direction, laid over the flank ──
    runs += _ink(_streams(wing, _cross(_flow, math.radians(-10)), 5.2), seq,
                 _shadow(tone, 0.24, 1.0), hold=0.24, gape=0.45, wander=0.35)
    ...
    for axis, ws in ((_LEG_A, _LEG_A_W), (_LEG_B, _LEG_B_W)):
        ...
        legs += _ink(_streams(fld, flow, 2.0, step=2.4, max_pts=520), seq, lt,
                     hold=0.06, gape=0.62, wander=0.14)
        legs += _ink(_streams(fld, _cross(flow, math.radians(78)), 3.2, step=1.1, max_pts=40),
                     seq, _shadow(lt, 0.38), hold=0.34, gape=0.4, wander=0.12)

    # The toes are cut the way the legs are, not as drawn lines: at the opacity this mark lives at,
    # a single dashed stroke is invisible and the bird stops standing on anything. Narrow ribbons,
    # hatched down their own axis, give them just enough body to register as contact.
    feet = []
    for toe in _TOES:
        centre = _catmull(toe, closed=False, steps=12)
        half = _widths(toe, [5.4, 3.6, 2.4, 1.8][:len(toe)], 12)
        fld = _Field(_thick(centre, half), step=0.6, pad=1.6)
        flow = _leg_flow(centre)
        feet += _ink(_streams(fld, flow, 1.8, step=1.8, max_pts=160), seq,
                     lambda x, y: 0.97, hold=0.05, gape=0.7, wander=0.1)
    ...
    dots = _stipple(body, seq, tone, 38000, gain=0.62, bias=1.6)
...
_WEIGHTS = ((0.46, 1.05), (0.68, 1.35), (1.01, 1.68))
...
def _body(plate) -> str:
    ...
    for key, scale in (("runs", 1.0), ("legs", 1.02)):
    ...
    for group, (_, w) in zip(_bucket(plate["feet"]), _WEIGHTS):
        if group:
            parts.append(f'<path stroke-width="{w:.2f}" d="{"".join(_pass(p) for p in group)}"/>')
    parts.append(f'<path stroke-width="1.6" d="{_points(plate["dots"])}"/>')
