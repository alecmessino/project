...
*Frozen. This defines the operating system before we design its first screen. The onboarding
...
amendment. If an interface and this document disagree, the interface is wrong.*
...
| 4 · What would reopen it | **Trigger** | Monitored continuously; checked at every review |

## The Decision Record — canonical schema

The unit of judgment. Seven fields; three carry the judgment and are non-negotiable.
...
| **Trigger** | The condition that reopens the decision | **What makes review governance, not narrative** |
...
- **Trigger** is what converts the annual review from a re-read into an auditable act: *did the
  condition fire?* A record without a trigger is incomplete.
...
5. **Every record carries a trigger.** A decision with no stated condition to revisit it is not yet
   a complete record.
6. **The Decision Log is the single source of truth.** The Manual, the AWOR, the Operating Sessions,
   and any future application are interfaces onto it. Interfaces change; the record does not.
...
**Frozen:** the four invariant questions, the record schema (especially Rationale, Trigger,
Supersession), the four-state lifecycle and its transitions, the responsibility of each artifact,
