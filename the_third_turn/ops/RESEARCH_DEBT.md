...
| **RD-3** | `book_panel` mixes **main and alternate total lines** with no discriminator (~95% of (game,book,ts) groups carry 2–3 lines). | Any line-based inference: leadership, drop/reversion, SR-3 encompassing robustness | Conflating distinct contracts (the main total vs alt strips) makes "the line moved" ambiguous, can double-count "simultaneous pairs," and would bias any signal keyed on the live line. | Untriaged; the SR-1 pairs count's exposure to alt-line fan-out is not yet quantified. | ED-3 |
...
| **RD-8** | **The two live books are not interchangeable instruments.** Measured 2026-07-19 (`BOOK_CHARACTERIZATION.md`): FanDuel re-prices ~every 31 s with vig IQR 0.36 pp (high-frequency, tight); Bovada re-prices ~every 8 min with vig IQR 1.80 pp (coarse, sticky). | Any cross-book statistic — consensus line, leadership, divergence, encompassing robustness | The project historically treated books as interchangeable. A consensus or leadership estimate that weights two behaviorally different instruments symmetrically is mis-specified: the granular book dominates any "who moved" count purely by sampling density, and a naive median blends a live tape with a slow anchor. Provisionally **falsifies** the interchangeability assumption (A-register). | Characterized, not yet modeled. Any cross-book analysis must weight by instrument behavior or restrict to one book; leadership needs the event-aligned test (deferred). | ED-1, RD-2, RD-4 |
