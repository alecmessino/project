...
| 17 | $14,800/yr — Asset location (register sample row) | `hub.html` "Coordination, kept in one register" | No Python function; hardcoded `<td>`. Adjacent disclosure: *"Illustrative sample, a fictional household. Potentials are modeled on the household's own figures."* | **ASSUMED** — weak: labeled illustrative, but the specific household/rate assumptions behind $14,800 are not stated or reproducible. **Phase 2: pin a canonical illustrative household, publish its assumptions, add a pinning test** (e.g. against `taxlab.location_alpha3()`, which yields a plausible but not currently-matching range for realistic $2–5M households — do not reverse-fit parameters to hit $14,800; derive honestly and republish the number that falls out) |
| 18 | $4,100/yr — Roth conversion window (register sample row) | `hub.html`, same table | Same as #17: hardcoded, same adjacent disclosure. `taxlab.py::roth_conversion()` exists and is tested but is not wired to this row | **ASSUMED** — same weak-caveat and Phase 2 item as #17 |
| 19 | $11,300 kept — Loss carryforward (register sample row, "Resolved") | `hub.html`, same table | Same as #17: hardcoded, same adjacent disclosure. `src/drift/tax.py::after_tax_track` computes a generic `carry_loss` but nothing ties it to this figure | **ASSUMED** — same weak-caveat and Phase 2 item as #17 |
...
| 23 | Homepage hero: "that gap was $320,000" | `hub.html` "Measure" step | Same $410,000 − $90,000 = $320,000 as #22 (static prose, not templated) | **ASSUMED** — same drift-risk note as #22 |
...
2. **Rows 17–19** — pin a canonical illustrative household for the `hub.html` register sample, publish its
   assumptions, add a pinning test. Do not reverse-fit parameters to hit the current $14,800/$4,100/$11,300
   — derive honestly and republish whatever number results.
