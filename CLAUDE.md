...
- **The house mark is governed, not decorative.** The heron engraving
  (`src/drift/web/img/heron-engraving.svg`, generated by `src/drift/heron.py` via
  `scripts/build_heron.py`) is a *house mark*, not a logo: one master, reused verbatim, and
  allowed only where Driftwood makes an enduring statement — today, the homepage hero, and
  nowhere else. Never nav, footer, favicon, dividers, avatars, or any repeating slot; its rarity
  is the entire instrument. Rules and rationale in `OPERATIONS.md`, enforced by
  `tests/test_drift_heron.py`.
