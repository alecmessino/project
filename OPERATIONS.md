...
**What replaced it, and why it was the right trade.** The hero watershed: the Mississippi system as
pure vector structure on limestone — 24 capillaries, four majors, one stem, no fill, no silhouette,
no labels, no city dots, no borders, no coastline. Twenty-four small waters begin apart, join into
four majors, and resolve into one channel that is larger below every join than above it. That is
the firm's claim about coordination, drawn rather than stated, and it is the reason the trade is
sound: **the heron's job in that plate was atmosphere; the watershed's job is the argument.** The
heron said composure. The watershed says what the composure is *for*. Two atmospheric marks in one
hero would have made both quieter, so the mark left the plate rather than sharing it.

The geometry is generated, not traced — `design/hero-watershed-2026/build_paths.py` projects short
(lat, lon) landmark lists through one equirectangular projection holding the true 1.31
latitude-to-longitude ratio at 40°N, then converts centripetal Catmull-Rom to exact cubic beziers.
Every path and every animation delay in `hub.html` comes from that script via `--inject`; the same
run writes the prototypes in that directory, so the live hero and the lab can never drift apart.
Hand-editing a `d` attribute, or anything between the `GEOM` markers, is the one thing not to do.

Stroke weight is a single dial, `--w` on `.ws` (1.6 desktop, 1.35 stacked). Every stroke is a
multiple of it, so the drawing scales in proportion and the hierarchy never shifts: capillary,
major and stem stay at exactly 1 : 1 : 2 at rest. Resting vectors are `#4a86b8` at 0.85 × `--w`
and 0.9 opacity; the stem is editorial blue at 1.7 × `--w`. Hover resolves the hierarchy — outer
thins and falls back, majors come up, the stem swells and pulses a soft blue shadow.

**Motion: this slot now loops, and that is a deliberate change to the doctrine.** The rule for the
house mark was *permanent, not animated* — one pass, then nothing, no loop, no scroll replay, no
drift, no breathing. That rule governed a mark whose whole claim was permanence. The watershed's
claim is different: it is about flow arriving and converging, and a still drawing states that
without demonstrating it. So the plate carries a 12s continuous linear cycle — a single lit segment
crossing each vector, phased so the wave runs outer → majors → stem, the small waters lighting
first and the stem carrying it to the delta last. The network itself is *always fully drawn*; only
the front moves. An earlier build animated the visible line itself, which is the textbook
continuous draw and leaves each vector absent for half of every cycle — a hero cannot be empty a
third of the time it is on screen. Confluence pulses are transient (one per cycle, nothing left
behind, so the drawing never acquires a city dot), the stem breathes on the same 12s clock so the
two never beat, and reduced motion keeps the entire network and drops only the movement.

**The doctrine now reads: permanence is a property of the house mark, not of the hero slot.** If
the mark ever returns to a page, it returns under its own rule, unchanged.
