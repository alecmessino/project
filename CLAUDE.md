...
- **Homepage 1a and the six editorial pages are locked (2026-09-06).** The approved review ZIP
  supplies the visual direction. The homepage uses `img/heron-plate.svg` verbatim, in its own
  right column; do not regenerate it or add heron variants. The old generated house mark remains
  archived in `design/` and is not used by the website. Register second, interactive lattice third.
- **Shared chrome:** edit `src/drift/nav.py`, run the existing nav installer, then `sync_docs.py`.
  The sync includes chrome on editioned Atlas pages without changing their data. Fees and For
  professionals remain top-level; Insights folds under The record near 1200px. The existing
  dropdown/menu keyboard behavior remains in `dw-context.js`.
- **Verification:** `python scripts/check_site_merge.py` checks all seven locked pages at eight
  widths plus navigation, lattice interactions, no-JS access, and Manual print. Optional
  `--browser /path/to/chromium` uses an installed browser. Artifacts live under `artifacts/site-merge`.
- **External follow-ups:** a confirmed fee-schedule PDF is not available. Keep its label
  non-interactive and explicitly unpublished until supplied; never link placeholder disclosures.
  Professional bookings retain the live event URL with the separate `cpa_referral` campaign until
  a dedicated event is confirmed.
