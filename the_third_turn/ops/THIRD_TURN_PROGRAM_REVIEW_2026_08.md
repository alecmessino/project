...
> **Correction, 2026-08-27 (this finding was wrong, and is preserved above as written).** The
> isolation test behind it restored the committed PNGs *before* running `make_figures.py`, so three
> files that `make_figures.py` never writes were read as "regenerated byte-identically".
> `make_concept_figures.py` is the **sole** producer of `appendix_vig.png`,
> `concept_encompassing.png` and `concept_laboratory.png`. Re-verified by deleting all three and
> running `make_figures.py` alone: none reappears. The prescribed fix -- dropping the script from the
> documented path -- was applied, and it left three of Paper 1's eleven figures with no generator at
> all. `release/README.md` and `paper/REPRODUCE.md` now name all four generators. The underlying L1
> defect (published instructions that do not reproduce the committed figures) was real; only the
> diagnosis and the remedy were not.
